...
class SnakeBlock:
    def __init__(self, _pos: tuple[int, int], _parent: typing.Optional['SnakeBlock'], _sprite_sheet, _dims: tuple[int, int ]=(20,20), _curr_dir: int=1) -> None:
        '''
    ...
    def follow(self, speed: list[int, int]):
        '''
        Follow your parent block if you're not the head, else just move in your curr_dir
        '''
        if self.parent:
            x1, y1 = self.pos
    ...
    def draw(self, surf: pygame.Surface):
        '''
        Draw the snakeblock at the current position.
        '''
        # self.box = pygame.draw.rect(surf, 'green' if self.parent else 'red', pygame.Rect(self.pos, self.dims))

        # based on curr_dir, draw the corrects sprite
        head_or_body = self.dims[1] if self.parent else 0
        match self.curr_dir:
            case 0:
                sprite_rect = pygame.Rect(0, head_or_body, self.dims[0], self.dims[1])
            case 1:
                sprite_rect = pygame.Rect(self.dims[0], head_or_body, self.dims[0], self.dims[1])
            case 2:
                sprite_rect = pygame.Rect(2*self.dims[0], head_or_body, self.dims[0], self.dims[1])
            case 3:
                sprite_rect = pygame.Rect(3*self.dims[0], head_or_body, self.dims[0], self.dims[1])

        self.box = surf.blit(self.sprite_sheet, self.pos, sprite_rect)
...
def main_game_loop(screen: pygame.Surface, snake_sprite: pygame.Surface, block_dims: tuple[int, int]):
    # start pygame
    pygame.init()
    pygame.font.init()
    ...
    head = SnakeBlock(start_position, None, snake_sprite, block_dims)
    mid = SnakeBlock((start_position[0] - block_dims[0], start_position[1]), head, snake_sprite, block_dims)
    mid1 = SnakeBlock((start_position[0] - 2*block_dims[0], start_position[1]), mid, snake_sprite, block_dims)
    mid2 = SnakeBlock((start_position[0] - 3*block_dims[0], start_position[1]), mid1, snake_sprite, block_dims)
    tail = SnakeBlock((start_position[0] - 4*block_dims[0], start_position[1]), mid2, snake_sprite, block_dims)
    ...
    pygame.draw.rect(movable_area, screen_color, movable_area.get_rect(), width = thic)
    ...
    while running:
        for event in pygame.event.get():
            ...
            if fruit_collected >= 0:
                ...
                new_mid = SnakeBlock(tail.pos, tail.parent, snake_sprite, block_dims, tail.curr_dir)
            ...
            movable_area.fill(movable_area_color)
